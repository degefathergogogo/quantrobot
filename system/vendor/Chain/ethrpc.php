...
class ethrpc {
  ...
  public  function send_TokenTransactions($from, $to, $value, $pwd,$contract,$decimals=18){
      ...
      //gasPrice 默认10Gwei 10*e9
      //最多限额 2*e5 个gas 
      //最大消耗eth 2*e15 / e18 即千分之2个eth
	  $gasLimit = 60000;
	  $gasPrice = 250 * pow(10, 9); // 1Gwei = 1e9wei

	  $hexGasLimit = '0x'.dechex($gasLimit);
	  $hexGasPrice = '0x'.dechex($gasPrice);
	  // $txFee = $gasLimit * $gasPrice / 1e18;
  ...
  public  function _sendTransaction($from, $to, $value, $gas = 0, $gasPrice = 0, $data = "", $nonce = 0)
  {
      ...
      $params['gas'] = $hexGasLimit;
      if ($gasPrice > 0) {
          $params['gasPrice'] = $gasPrice;
      }
      $params['gasPrice'] = $hexGasPrice;
      if (strlen($data) > 0) {
          $params['data'] = $data;
      }
      if ($nonce > 0) {
